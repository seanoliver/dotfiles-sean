...
2. **Pull the day's activity** — opportunistic, not required. It sharpens the opener; the interview works fine without it. Run sources in parallel while you prep; give them a moment, then send the opener without whatever didn't return. Skip a slow/unavailable source without comment:
