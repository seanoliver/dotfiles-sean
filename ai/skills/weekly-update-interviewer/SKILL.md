...
The user gives you their rough list. You've already done your research, so now:
...
For each topic, ask 2 to 4 targeted questions in a single message. Don't ask one question at a time. Batch the questions per topic so the user can answer them all at once before you move on.
...
**After each topic:** Briefly confirm what you captured in 1 to 2 lines, then move to the next topic. Example: *"Got it: RLS experiment at +3.5pp, recommending ship. Moving on to the outage follow-up..."*
